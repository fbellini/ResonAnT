...
Bool_t isFitSuccessful(Int_t result)
{
  //fitStatus = migradResult + 10*minosResult + 100*hesseResult + 1000*improveResult
  int improveResult = result/1000;
  int hesseResult = (result%1000)/100;
  int minosResult = ((result%1000)%100)/10;
  int migradResult = (result%10);

  if (migradResult>0) {
    return kFALSE;
  } else {
    if (improveResult) {
      Printf("IMPROVE exits with error");
    }
    if (hesseResult){
      Printf("HESSE exits with error");
    }
    if (minosResult){
      Printf("MINOS exits with error");
    }
  }
  return kTRUE;
}
...
TH1 * YieldMean(TH1 *hstat, TH1 *hsys, TF1 *f = NULL, Double_t min = 0., Double_t max = 10., Double_t loprecision = 0.01, Double_t hiprecision = 0.1, Option_t *opt = "I0q",TString logfilename="log.root", Double_t minfit=0.0, Double_t maxfit=10.0, Double_t * fitChi2ndf = NULL, Double_t * fitParMain= NULL, Double_t * fitParErrMain = NULL)
{
  if(maxfit>max)
    max=maxfit;	
  if(minfit<min)
  ...
  do { 
    ...
    TFitResultPtr fitresptr = htot->Fit(f, opt,"",minfit,maxfit);
    for (Int_t ipar=0; ipar < 5; ipar++) {
      fitParMain[ipar] = fitresptr->Parameter(ipar);
      fitParErrMain[ipar] = fitresptr->ParError(ipar);
      Printf("par %i: %f +/- %f", ipar, fitParMain[ipar], fitParErrMain[ipar]);
    }
    fitres = fitresptr->Status();
    /**************************************** end of fbellini's hack *******************************/ 
  ...
  /******* fbellini's hack to get parameters of the fit function for the central value************/
  if (fitChi2ndf) {
    fitChi2ndf[0] = f->GetChisquare();
    fitChi2ndf[1] = f->GetNDF();
  } else {
    fitChi2ndf[0] = 0.0;
    fitChi2ndf[1] = -1.E03;
  }
  ...
  while (!isFitSuccessful(fitres));
  hlo = YieldMean_LowExtrapolationHisto(hstat, f, min, loprecision, integrationStep);
  hhi = YieldMean_HighExtrapolationHisto(hstat, f, max, hiprecision, integrationStep);
...
TH1 *
YieldMean_LowExtrapolationHisto(TH1 *h, TF1 *f, Double_t min, Double_t binwidth, Double_t integrationStep)
{
  ...
  for (Int_t ibin = 0; ibin < hlo->GetNbinsX(); ibin++) {
    ...
    cont = f->Integral(hlo->GetBinLowEdge(ibin + 1), hlo->GetBinLowEdge(ibin + 2), integrationStep);
    err = f->IntegralError(hlo->GetBinLowEdge(ibin + 1), hlo->GetBinLowEdge(ibin + 2), (Double_t *)0, (Double_t *)0, integrationStep);
...
TH1 *
YieldMean_HighExtrapolationHisto(TH1 *h, TF1 *f, Double_t max, Double_t binwidth, Double_t integrationStep)
{
  ...
  for (Int_t ibin = 0; ibin < hhi->GetNbinsX(); ibin++) {
    ...
    cont = f->Integral(hhi->GetBinLowEdge(ibin + 1), hhi->GetBinLowEdge(ibin + 2), integrationStep);
    //    cont = f->Integral(hhi->GetBinLowEdge(ibin + 1), hhi->GetBinLowEdge(ibin + 2), (Double_t *)0, 1.e-6);
    err = f->IntegralError(hhi->GetBinLowEdge(ibin + 1), hhi->GetBinLowEdge(ibin + 2), (Double_t *)0, (Double_t *)0, integrationStep);
