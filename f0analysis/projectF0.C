...
TString macroDir ="$HOME/alice/macro/ResonAnT/"; 

void projectF0
(
   const char *nameData = "RsnTask_f0.root",
   const char *outName  = "kstar",
   const char *listName = "RsnOut_f0",
   Char_t *icut = "66",
   Bool_t isPP= 0 ,
   Bool_t isMC = 0
)
{
  ...
  TList *out = new TList();
   ...
   if (listData) {
      hInput[ 0] = (TH3F*)listData->FindObject("RsnTaskF0_f0_UnlikePM");
      hInput[ 1] = (TH3F*)listData->FindObject("RsnTaskF0_f0_MixingPM");
      hInput[ 2] = (TH3F*)listData->FindObject("RsnTaskF0_f0_LikePP");
      hInput[ 3] = (TH3F*)listData->FindObject("RsnTaskF0_f0_LikeMM");
      // hInput[ 4] = (TH3F*)listData->FindObject("RsnTaskF0_f0_UnlikeMP");
      // hInput[ 5] = (TH3F*)listData->FindObject("RsnTaskF0_f0_MixingMP");
      ...
      // hInput[ 4]->SetName("Data_UnlikeMP");
      // hInput[ 5]->SetName("Data_MixingMP");
   
   }
     
   // define binning in pT    
   /* use binning as for phi */
   Double_t cent[]={ 0.0, 100.0 };// 20.0, 40.0, 60.0, 80.0, 90.0};   
   ...
   for (Int_t icentbin=0; icentbin<ncent;icentbin++){
     ...
     // lUnlikeMP.Write(hInput[4]->GetName(), TObject::kSingleKey);
     // lMixingMP.Write(hInput[5]->GetName(), TObject::kSingleKey);
     fout->Close();
   }
