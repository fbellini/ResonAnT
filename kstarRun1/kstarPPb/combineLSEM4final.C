void combineLSEM4final(Bool_t useSmoothed=1, Bool_t plotPreliminary=0)
{
  gStyle->SetOptStat(0);
  ...
  Color_t color[8]={ kOrange+1, kSpring+5, kTeal-5, kAzure+1, kBlue+1, kBlack, kMagenta, kOrange};  
    // kRed+1, kOrange-1, kCyan+2, kViolet-3, kAzure+7, kBlack, kMagenta, kOrange}; //combined final (as the phi)
    // kRed+1, kOrange-2 /*Kpink+6*/, kGreen+2, kAzure+1, kBlue+3, kBlack, kMagenta, kOrange};  
  Color_t colorsys[9]={ kRed, kBlue+1, kGreen+1, kMagenta+1, kOrange, kCyan+2, kPink+2, kGray, kViolet+1};  
  Int_t markers[5] = {20,23,29,33,34};

  //get bins
  TString filebins = "$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/multi/proj_2424_tpc2s_tof3sveto.root";
    //final from oct. 2014 "$HOME/alice/resonances/kstar_pA5.02TeV/LF_pPb_8-9/multi_binB/proj_2424_tpc2s_tof3sveto.root";
  ...
  //access files with min bias spectra EM FINAL 
  TFile * fin100EM;
  if (useSmoothed) 
    //oct 2015
    fin100EM = TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/NSD/finalWsyst_smooth2_EM_12oct15_0.root");
    //fin100EM = TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/NSD/finalWsyst_smooth2_EM_01jul15.root");
    //mar 2015 TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/LF_pPb_8-9/0to100_binB/final/finalWsyst_smooth2_EM_28mar15_0.root");
  else 
    fin100EM = TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/NSD/finalWsyst_EM_01jul15.root");
  ...
  ApplyCosmetics2Spectrum(hCorrMinBiasEM, color[5], 24); 
  ...
  hCorrMinBiasEMsys->SetTitle(Form("NSD"));
  ctmp->cd();
  ...
  //access files with min bias spectra LS FINAL
  TFile * fin100LS;
  if (useSmoothed) 
    fin100LS = TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/NSD/finalWsyst_smooth2_LS_12oct15_0.root");
  //fin100LS = TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/NSD/finalWsyst_smooth2_LS_01jul15.root");
      //TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/LF_pPb_8-9/0to100_binB/final/finalWsyst_smooth2_LS_28mar15_0.root");
  else 
    fin100LS = TFile::Open("$HOME/alice/resonances/kstar_pA5.02TeV/output_LF5455/NSD/finalWsyst_LS_01jul15.root");
  ...
  ApplyCosmetics2Spectrum(hCorrMinBiasLS, color[5], 24); 
  ...
  hCorrMinBiasLSsys->SetTitle(Form("NSD"));
  ...
  for (Int_t ic=0;ic<5;ic++) {
    ...
    if (plotPreliminary) {
      if (useSmoothed) 
     	finMultiCorrEM[ic] = TFile::Open(Form("$HOME/alice/resonances/kstar_pA5.02TeV/LF_pPb_8-9/multi_binB/preliminary/finalWsyst_smooth2_EM_27apr14_%i.root", ic));
      else
    ...
    if (plotPreliminary) {
      if (useSmoothed) 
	finMultiCorrLS[ic] = TFile::Open(Form("$HOME/alice/resonances/kstar_pA5.02TeV/LF_pPb_8-9/multi_binB/preliminary/finalWsyst_smooth2_LS_27apr14_%i.root", ic));
      else    
    ...
    if (!finMultiCorrLS[ic]) { Printf(Form("Error in reading file %s", finMultiCorrLS[ic]->GetName())); return;}
    if (!finMultiCorrEM[ic]) { Printf(Form("Error in reading file %s", finMultiCorrEM[ic]->GetName())); return;}
    ...
    ApplyCosmetics2Spectrum(hCorrMultiEM[ic], color[ic], markers[ic]);
    ...
    ctmp->cd();
    TExec *setex7 = new TExec("setex7","gStyle->SetErrorX()");
    setex7->Draw();
 ...
 for (Int_t ic=0; ic<ncent; ic++) {
   ...
   //sys uncert uncorr hadrons
    ...
    gPad->SetTickx();
    gPad->SetTicky();
    TExec *setex0 = new TExec("setex0","gStyle->SetErrorX()");
    setex0->Draw();
    hFinalMultiSys[ic]->Draw("E2same");
  ...
  TLegend * leg = new TLegend(0.55,0.5,0.89,0.78);
 leg->AddEntry(hFinalMinBias, hFinalMinBias->GetTitle(), "p");
 for (Int_t ic=0;ic<5;ic++){ 
    leg->AddEntry(hFinalMulti[ic], hFinalMulti[ic]->GetTitle(), "p");
 }
 ...
 cfinalMulti->SaveAs("final_Ks_pPb_multi.png");
 cfinalMulti->SaveAs("final_Ks_pPb_multi.pdf");
 cfinalMulti->SaveAs("final_Ks_pPb_multi.C");
  //save into filehCorrected_44_syst_uncorrhCorrected_44_syst_uncorr
  TFile * fout;
  if (plotPreliminary) fout = new TFile(Form("prelim_kstar_pPb%s.root", useSmoothed?"_smoothSys":""),"recreate");
  else fout = new TFile(Form("finalCand_kstar_pPb%s.root", useSmoothed?"_smoothSys":""),"recreate");
  fout->cd(); 
...
void ApplyCosmetics2Spectrum(TH1D* h, Color_t color, Int_t marker=-1, Int_t lineWidth = 1)
{
  ...
  h->GetYaxis()->SetTitle("1/N_{evt}d^{2}#it{N}/(d#it{p}_{T}d#it{y}) [(GeV/#it{c})^{-1}]");
...
TPaveText * AddPaveText_KStar_pPb(TString position = "bl"){
  ...
  if (position.Contains("tl")) pave = new TPaveText(0.12,0.75,0.45,0.89,"NDC");
    ...
    if (position.Contains("bl")) pave = new TPaveText(0.22,0.22,0.55,0.36,"NDC");
      ...
      if (position.Contains("tr")) pave = new TPaveText(0.50,0.78,0.89,0.88,"NDC");
      else
...
TPaveText * AddPaveTextErrors(TString text="uncertainties: stat.(bars), syst.(boxes)"){
  TPaveText * pave = new TPaveText(0.15,0.2,0.63,0.2,"NDC");
  pave->SetBorderSize(0);
  pave->SetFillColor(kWhite);
  pave->SetFillStyle(0);
  pave->SetTextColor(kBlack);
