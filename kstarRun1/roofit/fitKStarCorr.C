	      ...
	      )
{
  ...
  for (Int_t icentbin=0; icentbin<ncent;icentbin++){   
    ...
    TString nameimg1 = Form("img/%s_c%i_pt%i.png", function.Data(),icentbin,istartptbin);
    TString nameimg2 = Form("img/%s_c%i_startpt%i.png",function.Data(),icentbin,istartptbin);
    if (infilename.Contains("_antikstar_")) nameimg1.ReplaceAll("img/","img/antikstar_");
    else
      if (infilename.Contains("_kstar_")) nameimg1.ReplaceAll("img/","img/kstar_");
      else
	if (infilename.Contains("_sum_")) nameimg1.ReplaceAll("img/","img/sum_");
		  ...
		  )
{
  ...
  */
  ...
  const Double_t showxrangem = 0.64,showxrangeM = 1.3;
  Double_t res=0.003,fitResMin=0.001,fitResMax=0.050; //Voigtian resolution param
  
  RooRealVar x("m","M (GeV/c^{2})", 0.64, 1.3);//
  ...
  // RooNumConvPdf lxg("lxg","landau x gauss",x,landau,gauss) ; 
   ...
   //Display data
   RooPlot * xframe = x.frame(0.6,1.5);//showxrangem, showxrangeM);
...
if (function.Contains("BOLTZ")){
 ...
 if (function.Contains("BWPS3")){
   // Construct Chi2
   data->Print("v") ;
   x.setRange("chi2range", xrangem, xrangeM);
   // RooChi2Var fitchi2("fitchi2","fitchi2", modelRPS3, data, kTRUE/*extended*/,"chi2range");
   ...
   // m2.migrad();
   // m2.hesse();
   // //m2.minos(); //analog of "E" option in root TH1::Fit()
   // //   m2.improve(); //analog of "I" option in root TH1::Fit()
   // RooFitResult* r2 = m2.save() ;
   // cout << "result of chi2 fit" << endl ;
   // r2->Print("v") ;
  ...
  // RooFitResult* r1 = m1->save() ;
   ...
   modelRPS3.chi2FitTo(data, Extended(kTRUE), SumW2Error(kFALSE), Range(xrangem, xrangeM));
   ...
   //get correction factor on signal for narrow fit range
   x.setRange("fitRange", xrangem, xrangeM) ;
   x.setRange("fullRange",0.6,1.5) ;
   x.setRange("signal3sRange", 0.8315, 0.9605) ;
   RooAbsReal* imodelfull = modelRPS3.createIntegral(x);
   RooAbsReal* imodelfull2 = modelRPS3.createIntegral(x, NormSet(x), Range("fullRange")) ;
   RooAbsReal* imodelfit = modelRPS3.createIntegral(x, NormSet(x), Range("fitRange")) ;
   RooAbsReal* imodel3s = modelRPS3.createIntegral(x, NormSet(x),Range("signal3sRange")) ;
   Double_t val_imodelfull = imodelfull->getVal();
   Double_t val_imodelfull2 = imodelfull2->getVal();
   Double_t val_imodelfit = imodelfit->getVal();
   Double_t val_imodel3s = imodel3s->getVal();

   RooAbsReal* ifull = relbwps.createIntegral(x);
   RooAbsReal* ifull2 = relbwps.createIntegral(x, NormSet(x),Range("fullRange")) ;
   RooAbsReal* ifit = relbwps.createIntegral(x, NormSet(x), Range("fitRange")) ;
   RooAbsReal* i3s = relbwps.createIntegral(x, NormSet(x),Range("signal3sRange")) ;
   Double_t val_ifull = ifull->getVal();
   Double_t val_ifull2 = ifull2->getVal();
   Double_t val_ifit = ifit->getVal();
   Double_t val_i3s = i3s->getVal();
  
   Printf("=== modelRPS3 I = %e \n === modelRPS3 I(full) = %e \n === modelRPS3 I(fit) = %e \n === modelRPS3 I(3sigma) = %e \n", val_imodelfull, val_imodelfull2, val_imodelfit, val_imodel3s);
   Printf("=== RBWPS I = %e \n === RBWPS I(full) = %e \n === RBWPS I(fit) = %e \n === RBWPS I(3sigma) = %e \n", val_ifull, val_ifull2, val_ifit, val_i3s);
 
   Double_t corr4fitRange = val_ifit/val_ifull2;
   Printf("=== RBWPS signal correction factor due to the choice of the fit range f = %5.3f (missing %3.2f%%)",corr4fitRange, (1-corr4fitRange)*100.);

   //TF1 from RooAddPdf of signal only
   // RooArgSet * obs  = modelRPS3.getObservables(*data);
   // RooArgSet * par  = modelRPS3.getParameters(*data);
   // parvv->Print("V");
   // RooArgList p (*parvv);
   
   // // take normalized pdf 
   // TF1 * fnormS = modelRPS3.asTF(*obs,p, *obs );
   
   // Double_t binWidth= hnsig->GetBinWidth(1);
   // FitFunc wf( fnormS, binWidth );
   // TF1 * functotS = new TF1("fsignal", &wf, 2.,5.,p.getSize(), "FitFunc");
   // Double_t Nfull=functotS->Integral(0.,999.);
   // Double_t Nfit=functotS->Integral(xrangem,xrangeM);
   // Printf("*** Integral of TF1 signal in full range = %e \n Integral of TF1 signal in fit range = %e", Nfull, Nfit);
 }
 ...
 /*----------------------------------------
   ...
   /*----------------------------------------
     for chi2 fit
   -----------------------------------------*/
   RooChi2Var fitchi2("fitchi2","fitchi2",modelEXE,data,kTRUE/*extended*/) ;
   RooMinuit m2(fitchi2) ;
   m2.migrad();
   m2.hesse();
   m2.minos(); //analog of "E" option in root TH1::Fit()
