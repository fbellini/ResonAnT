...
//---------------------------------------------------------------------------------------
void systesi( Char_t * inlist=NULL,  Char_t * inlist2=NULL, Int_t selCent = 0, Float_t chi2cut=2.0, TString filebins="sub_analysisAOD_0-80.root", Bool_t display=0, TString suffix)
{
  ...
  TString name1 = makeTreeChain(inlist);
  TString name2 = makeTreeChain(inlist2);

  //display pt spectrum from list 1
  TH1F* spectrum1 = (TH1F*) MakeSpectraCent(EQuantity::kYields, selCent, name1.Data(), filebins.Data(), chi2cut);
  TString newname1 = Form("%s",name1.Data());
  ...
  TH1F* spectrum2 = (TH1F*) MakeSpectraCent(EQuantity::kYields,selCent, name2.Data(), filebins.Data(), chi2cut);
  TString newname2 = Form("%s",name2.Data());
  ...
  //define output
  TH1F*frameYieldVsPt=new TH1F("frameYieldVsPt","Raw yield vs. p_{t}; p_{t} (GeV/c); dN/dp_{t}", npt_axis, pt);
  TH1F*frameSystVsPt=new TH1F("frameSystVsPt","; p_{t} (GeV/c); syst. error", npt_axis, pt);

  TH1F* hYieldVsPt = new TH1F(Form("hYieldVsPt_%i",selCent),"stat. err. only; p_{t} (GeV/c); M (GeV/c^{2})", npt_axis, pt);
  hYieldVsPt->SetTitle(Form("centrality %s", centLabel.Data()));
  ...
  TH1F* hYieldVsPtWsyst = new TH1F(Form("hYieldVsPtWsyst_%i",selCent),"#sqrt{#sigma_{stat}^{2}+#sigma_{syst}^{2}}; p_{t} (GeV/c); M (GeV/c^{2})", npt_axis, pt);
  hYieldVsPtWsyst->SetTitle(Form("centrality %s", centLabel.Data()));
  hYieldVsPtWsyst->SetLineColor(color[selCent]+2);	       
  hYieldVsPtWsyst->SetMarkerColor(color[selCent]+2);	       
  hYieldVsPtWsyst->SetMarkerStyle(marker[selCent]);
  ...
  for (Int_t ientry = 0;ientry<nentries;ientry ++){
	...
	syst=1e-5;
      ...
      hYieldVsPt->SetBinContent(ptBinID1+1, yield1/dpt);
      hYieldVsPt->SetBinError(ptBinID1+1, erry1/dpt);
      hYieldVsPtWsyst->SetBinContent(ptBinID1+1, yield1/dpt);
      hYieldVsPtWsyst->SetBinError(ptBinID1+1, TMath::Sqrt(erry1*erry1/(dpt*dpt)+syst*syst));
  ...
  cry->Divide(2,1);
  cry->cd(1);
  frameYieldVsPt->GetYaxis()->SetRangeUser(9e2, 5e6);
  gPad->SetLogy();
  frameYieldVsPt->Draw();
