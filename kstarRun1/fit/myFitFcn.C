...
double myFitFcn::RelBreitWignerBoltzmann(double x, double * par)
{
  ...
  const Double_t mpi2 = mka2; //for phi // for K*: 0.1396*0.1396;
...
double myFitFcn::RelBreitWigner(double x, double *par)
{
  ...
  const double mkpi2 = TMath::Power(2.0*0.4937, 2.0);//for  K* (0.4937+0.1396)*(0.4937+0.1396); //GeV
...
double myFitFcn::bw1(double *x, double *par)
{
  ...
  const double MassPi = MassK; //0.13957;
