...
void projectKStarMCtrueCompare
(
   const char *nameData = "analysisAOD.root",
   TString listName = "RsnOut_quality",
...
)
{
  ...
  THnSparse* hInput[kNhistosData]= {0,0,0,0};
   ...
   //Double_t pt[] = {0.0, 0.5, 1.00, 1.50, 2.00, 2.50, 3.00, 3.5, 4.00, 4.5, 5.0, 6.0, 7.0, 8.0, 10.0};
   Double_t pt[] = {0.0, 1.00, 1.50, 2.00, 2.50, 3.00, 3.5, 4.00, 5.0, 7.0, 10.0};
   ...
   for (Int_t icentbin=0; icentbin<ncent;icentbin++){
     ...
     TH1F * hEffVsPt = new TH1F("hEffVsPt", "Efficiency for K*+#bar{K*}; p_{t} (GeV/c); #epsilon = reco true / generated", npt, pt);
     ...
     for (Int_t ih = 0; ih<lTruesPM.GetEntries();ih++){
       ...
       Int_t counts1 = ((TH1F*)lTruesPM.At(ih))->Integral();
       hTrueCountsPM->SetBinContent(ih+1, counts1);
       hTrueCountsPM->SetBinError(ih+1, TMath::Sqrt(counts1));
       ...
       Int_t counts2 = ((TH1F*)lTruesMP.At(ih))->Integral();
       hTrueCountsMP->SetBinContent(ih+1, counts2);
       hTrueCountsMP->SetBinError(ih+1, TMath::Sqrt(counts2));
       ...
       Printf("mother %i %s",ih, ((TH1F*)lMother.At(ih))->GetName());
       ...
       Int_t anticounts = ((TH1F*)lAntiMother.At(ih+npt))->Integral();
       Printf("antimother %i %s",ih+npt, ((TH1F*)lAntiMother.At(ih+npt))->GetName());
       hAntiMotherCounts->SetBinContent(ih+1, anticounts);
       hAntiMotherCounts->SetBinError(ih+1, TMath::Sqrt(anticounts));
     ...
     hEffVsPtKStar = (TH1F*)hTrueCountsPM->Clone("hEffVsPtKstar");  
     hEffVsPtKStar->Divide(hMotherCounts);
     
     hEffVsPtAntiKStar = (TH1F*)hTrueCountsMP->Clone("hEffVsPtAntiKstar");  
     hEffVsPtAntiKStar->Divide(hAntiMotherCounts);
     
     hEffVsPtKStar->GetYaxis()->SetRangeUser(0.0,1.0);
     hEffVsPtKStar->SetLineColor(kRed);
     hEffVsPtKStar->SetMarkerColor(kRed);
     hEffVsPtKStar->SetMarkerStyle(marker[icentbin]);
     
     hEffVsPtAntiKStar->GetYaxis()->SetRangeUser(0.0,1.0);
     hEffVsPtAntiKStar->SetLineColor(kBlue);
     hEffVsPtAntiKStar->SetMarkerColor(kBlue);
     hEffVsPtAntiKStar->SetMarkerStyle(marker[icentbin]);
     ...
     hEffVsPt->SetLineColor(color[icentbin]);
     hEffVsPt->SetMarkerColor(color[icentbin]);
     hEffVsPt->SetMarkerStyle(marker[icentbin]);
