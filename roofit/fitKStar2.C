	      ...
	      )
{
  ...
  for (Int_t icentbin=0; icentbin<ncent;icentbin++){   
	...
	ntree->SetBranchAddress("factor_em", &normfactor[0]);
    ...
    TString nameimg1 = Form("%s/img/%s_c%i_pt%i.png",outdirname.Data(), function.Data(),icentbin,istartptbin);
    TString nameimg2 = Form("%s/img/%s_c%i_startpt%i.png",outdirname.Data(),function.Data(),icentbin,istartptbin);
		  ...
		  )
{
  ...
  */
  ...
  // RooNumConvPdf lxg("lxg","landau x gauss",x,landau,gauss) ; 
   ...
   //Display data
   RooPlot * xframe = x.frame(xrangem, xrangeM);
...
if (function.Contains("BOLTZ")){
 ...
 if (function.Contains("BWPS3")){
   // Construct Chi2
   //data->Print("v") ;
   RooChi2Var fitchi2("fitchi2","fitchi2", modelRPS3, data, kTRUE/*extended*/);
   // Start Minuit session on Chi2
   RooMinuit m2(fitchi2) ;
   m2.setPrintLevel(2);
   //   m2.setVerbose();
