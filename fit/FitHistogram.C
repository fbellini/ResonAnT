...
(
   ...
   Double_t    pt       = 0.01,
   TString     opt      = "",
   Double_t    binCountMin  = 0.795,
   Double_t    binCountMax  = 0.995
)
{
   // prepare output
   ...
   case myFitFcn::kGaus:
     ...
     break;
   default:
     ::Error("FitSingleMix", "Wrong function type");
     return 0x0;
   }
   
   // draw & fit
   hist->Fit(result->fSum, "RQ0");
   gMinuit->Command("SET STRATEGY 2");
   hist->Fit(result->fSum, "ERQ0");
   ...
   Printf("sigma = %.6f  minSigma = %.6f maxSigma =%.6f",sigma,minSigma*sigma, maxSigma*sigma);
   ...
   if (opt.Contains("BC")){ 
     ...
     Printf("Integral = %e +/- %e", bgFitter->Integral(binCountMin,binCountMax), bgFitter->IntegralError(binCountMin,binCountMax));
...
#if 0
   // ofstream outtxt;
   // outtxt.open("fit1bin_phi_results.txt",ios::app);
   // if (outtxt.is_open()) {
   //   outtxt << (char*) hist->GetName() << "  " << fit->Functions() << "  " << viewMin << "  " << viewMax << "  " <<  sigma << "  " << minSigma << "  " << maxSigma << "  " << result->fSum->GetParameter(3) << "  " << result->fSum->GetParError(3) << "  " << result->fSum->GetChisquare(); << "  " << endl;
